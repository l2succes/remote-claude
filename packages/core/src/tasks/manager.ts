...
import { ComputeManager, ComputeConfig, ComputeProviderType } from '@remote-claude/core/providers/compute';
import { WebhookServer } from '@remote-claude/websocket';
import { NotificationManager, NotificationConfig, NotificationPayload } from '@remote-claude/notifications';
...
export class TaskManager extends EventEmitter {
  ...
  private async setupComputeProviders(options: TaskManagerOptions): Promise<void> {
    const { CodespaceProvider } = await import('@remote-claude/core/providers/compute/codespace-provider');
    const { EC2Provider } = await import('@remote-claude/core/providers/compute/ec2-provider');
